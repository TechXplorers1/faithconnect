...
export default function NewMinistryPage() {
  ...
  const { addMinistry } = useMinistries();
  ...
  function onSubmit(values: z.infer<typeof ministrySchema>) {
    const handleImage = (file: File, callback: (imageUrl?: string, imageId?: string) => void) => {
        if (file) {
            ...
            callback(undefined, `ministry-worship`);
        }
    };

    handleImage(values.image?.[0], (imageUrl, imageId) => {
        const newMinistry = {
            ...values,
            image: imageId || '',
            imageUrl: imageUrl
        };
        addMinistry(newMinistry);
        
        toast({
          title: 'Ministry Created!',
          description: `The ministry "${values.name}" has been successfully created.`,
        });
  ...
  return (
     <div className="w-full max-w-4xl mx-auto p-4 md:p-6 space-y-8">
       <div className="flex items-center gap-4">
        ...
        <h1 className="text-3xl font-bold tracking-tight font-headline">Create New Ministry</h1>
      </div>

      <Card>
        <CardHeader>
            <CardTitle>Ministry Details</CardTitle>
            <CardDescription>Fill in the details for your new ministry.</CardDescription>
        </CardHeader>
        <CardContent>
             <Form {...form}>
